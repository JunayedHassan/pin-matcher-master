...
document.getElementById('generate-pin').addEventListener('click', function(){
    const randomNumber = Math.floor(Math.random() *(9999 - 1000 + 1)) + 1000; // min 1000 max 9999 as given instruction. Formula source-- MDN.
    generateNumber.value = randomNumber;
    lessThan4.style.display = "none"; // to avoid any bug
});
...
function takeInput(id){
    if(displayNumber.value.length < 4){ // if input greater than 4 digit stop taking inputs
...
// reset button
    if(id == 'C'){
        displayNumber.value = '';
        notMatched.style.display = "none"; // same as 8th no line-- to avoid any bug
    }
}
